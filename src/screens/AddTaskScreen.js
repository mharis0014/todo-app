...
const AddTaskScreen = ({navigation, route}) => {
    ...
    try {
      ...
      if (edit) {
        const updatedTasks = tasks.map(t =>
          t.id === prevTask.id ? {...t, heading: task.title, text: task.description} : t,
        )
  ...
  return (
    <View style={[styles.fill, styles.p20]}>
      <Text style={styles.heading}>{edit ? 'Update Task' : 'Add New Task'}</Text>
      <TextInput
        value={task.title}
      ...
      <View style={styles.row}>
        <CustomButton
          ...
          onPress={addTask}
        />
        <CustomButton title={'Cancel'} color={theme.colors.dangerColor} onPress={goBack} />
      </View>
    </View>
  )
}
